...
(() => {
  ...
  (() => {
    ...
    function human(base) {
      ...
      if (Math.random() < 0.05) ms += 45e3 + Math.random() * 105e3;
    ...
    const DEFAULT_SEND_TIMEOUT_MS = 45e3;
    ...
    const RETRYABLE_SEND_TIMEOUT_TYPES = /* @__PURE__ */ new Set([
      "getTargets",
      "getXProfileTarget",
    ...
    const DIRECT_FETCH_CSP_BLOCKED = /* @__PURE__ */ new Set(["instagram", "threads", "facebook"]);
    function directFetchAllowed() {
      const p = currentPlatform();
      return !(p && p.id && DIRECT_FETCH_CSP_BLOCKED.has(p.id));
    }
    const DEFAULT_DIRECT_SEND_TIMEOUT_MS = 2e4;
    ...
    async function trySwFetchProxy(request) {
      try {
        const proxy = await withDeadline(
          ...
          DIRECT_SEND_TIMEOUT_MS_BY_TYPE.swFetchProxy || 15e3,
          "swFetchProxy message timed out after 15s",
          "UCSwFetchProxyTimeout"
        );
        if (proxy && proxy.ok !== void 0) {
          return {
            ok: !!proxy.ok,
    ...
    async function directSendFallback(msg, error) {
      ...
      try {
        const response = await withDeadline(
          ...
          timeoutMs,
          `${type} direct fallback timed out after ${Math.ceil(timeoutMs / 1e3)}s`,
          "UCDirectSendTimeout"
        );
      ...
      for (let i = 0; ; i++) {
        try {
          ...
          return await withDeadline(
            ...
            ms,
            `${type} message timed out after ${Math.ceil(ms / 1e3)}s`,
            "UCSendTimeout"
          );
        } catch (e) {
          ...
          if (!transient || i >= retries) {
            ...
            if (/Extension context invalidated/i.test(e.message || "")) return null;
            throw e;
          }
          await sleep(400);
        }
    ...
    const PAGE_HEALTH_REPORT_GAP_MS = 6e4;
    const WALL_LOG_GAP_MS = 10 * 6e4;
    const LAST_PAGE_HEALTH_REPORT = {};
    const LAST_WALL_LOG_AT = {};
    const PAGE_RECOVERY_ENABLED = /* @__PURE__ */ new Set(["facebook", "instagram", "lemon8", "tiktok", "threads", "x"]);
    ...
    function visiblePageText() {
      try {
        const focused = [...document.querySelectorAll('[role="alert"], [data-e2e*="error" i], [data-e2e*="empty" i], h1, h2, button')].map((el) => (el.innerText || el.textContent || "").trim()).filter(Boolean).slice(0, 80).join("\n");
        const body = document.body && document.body.innerText ? document.body.innerText.slice(0, 9e3) : "";
    ...
    function detectRecoverablePageShell(platformId) {
      ...
      if (platformId === "x") {
        const pageAgeMs = (() => {
          try {
            return Number(performance.now && performance.now()) || 0;
          } catch (e) {
            return 0;
          }
        })();
        const blankSpaShell = pageAgeMs > 2e4 && counts.articles === 0 && counts.videos === 0 && counts.links < 3 && String(text || "").trim().length < 20;
        if (blankSpaShell) {
          return {
    ...
    function findRecoverablePageActionButton(platformId) {
      ...
      return candidates.find((el) => {
        ...
        return /^(try again|retry|reload|refresh)$/i.test(text) || /\b(try again|retry|reload|refresh)\b/i.test(text);
      }) || null;
    }
    function switchXHostForRecoverableShell(shell) {
      ...
      const minNavGapMs = /try_again/i.test(shell.reason || "") ? 5 * 6e4 : 3 * 6e4;
    ...
    async function attemptRecoverablePageInteraction(platformId, shell) {
      ...
      if (platformId === "x" && shell.reason === "failed_script_url") {
        const key2 = "uc_recover_click_x_failed_script_url";
        const last2 = lsNum(key2);
        if (last2 && Date.now() - last2 < 2 * 6e4) {
          if (switchXHostForRecoverableShell(shell)) return true;
        }
        lsSet(key2, String(Date.now()));
        try {
          history.replaceState(null, "", "https://x.com/home");
        } catch (e) {
        }
        clog("warn", "x failedScript URL detected; returning to clean home", "x");
        location.href = "https://x.com/home?uc_recover=" + Math.floor(Date.now() / 1e3);
        return true;
      }
      const key = `uc_recover_click_${platformId}_${shell.reason || "shell"}`;
      const last = lsNum(key);
      if (platformId === "x") {
        if (last && Date.now() - last < 15e3) return false;
        lsSet(key, String(Date.now()));
        const xButtonKey = `uc_recover_x_button_${shell.reason || "shell"}`;
        const lastXButton = lsNum(xButtonKey);
        const button2 = findRecoverablePageActionButton(platformId);
        if (button2 && (!lastXButton || Date.now() - lastXButton > 2 * 6e4)) {
          lsSet(xButtonKey, String(Date.now()));
          try {
            button2.scrollIntoView({ block: "center", inline: "center" });
          } catch (e) {
          }
      ...
      if (last && Date.now() - last < 6e4) return false;
    ...
    function deepCollectMedia(obj, sink, entity, depth = 0) {
      ...
      coverSources.forEach((cover, i) => {
        const u = typeof cover === "string" ? cover : cover && (cover.urlList && cover.urlList[0] || cover.url_list && cover.url_list[0] || cover.url || cover.imageUrl || cover.image_url);
      ...
      if (Array.isArray(imgs)) {
        imgs.forEach((im, i) => {
          const u = im.imageURL && (im.imageURL.urlList || [])[0] || (im.urlList || [])[0] || im.url || im.imageUrl || im.imageInfos && im.imageInfos[0] && im.imageInfos[0].url;
    ...
    function deepCollectUsers(obj, users, depth = 0) {
      ...
      if (Array.isArray(obj)) {
        for (const v of obj) deepCollectUsers(v, users, depth + 1);
        return;
      }
      if (typeof obj !== "object") return;
      ...
      if (typeof uname === "string" && uname && uname.length < 40) {
        users.push({
          user_id: obj.id || obj.uid || obj.userId || null,
          username: uname,
      ...
      for (const k in obj) {
        const v = obj[k];
        if (v && typeof v === "object") deepCollectUsers(v, users, depth + 1);
      }
    }
    ...
    function cappedHuman(base, maxMs) {
      const ms = human(base);
      return Number.isFinite(Number(maxMs)) && Number(maxMs) > 0 ? Math.min(ms, Number(maxMs)) : ms;
    }
    async function autoScroll(times = 8, dist = 1400, pause = 1800, options = {}) {
      ...
      for (let i = 0; i < times; i++) {
        ...
        await sleep(cappedHuman(pause, maxPauseMs));
      }
    }
    ...
    function collectFollowHandlesFromDom(platform, owner) {
      ...
      const patterns = platform === "threads" ? [/^\/@([A-Za-z0-9._]{1,30})\/?$/] : platform === "tiktok" ? [/^\/@([A-Za-z0-9._]{1,32})\/?$/] : [/^\/([A-Za-z0-9_]{1,20})\/?$/];
    ...
    const SPIDER_FOLLOWS_PER_SIDE = 35;
    const IG_MAX_ITEMS = 180;
    function igStorySweepCap() {
      return lsBoundedInt("ucIgStorySweepCap", 6, 5, 8);
    }
    function igDeepProfileCap() {
      return lsBoundedInt("ucIgDeepProfileCap", 3, 2, 5);
    }
    function igRestWindowMs() {
      return lsBoundedInt("ucIgRestWindowMinutes", 10, 8, 22) * 6e4;
    }
    function igFamousFollowerCap() {
      return lsBoundedInt("ucIgFamousFollowerCap", 3e3, 1e3, 5e3);
    }
    function igRevisitHourlyCap() {
      return lsBoundedInt("ucIgRevisitHourlyCap", 1, 0, 3);
    }
    ...
    const IG_SEEDED_ACCOUNTS = /* @__PURE__ */ new Set();
    const instagram = {
      id: "instagram",
      ...
      postMeta(n) {
        const caption = n.edge_media_to_caption && n.edge_media_to_caption.edges && n.edge_media_to_caption.edges[0] && n.edge_media_to_caption.edges[0].node.text || n.caption && (n.caption.text || (typeof n.caption === "string" ? n.caption : null)) || null;
        ...
        return {
          ...
          music_title: n.clips_music_attribution_info && n.clips_music_attribution_info.song_name || n.music_metadata && n.music_metadata.music_info && n.music_metadata.music_info.music_asset_info && n.music_metadata.music_info.music_asset_info.title || null,
          music_author: n.clips_music_attribution_info && n.clips_music_attribution_info.artist_name || n.music_metadata && n.music_metadata.music_info && n.music_metadata.music_info.music_asset_info && n.music_metadata.music_info.music_asset_info.display_artist || null
        };
      },
      extractMedia(node, username) {
        ...
        const push = (n, cid2) => {
          ...
          if (url) out.push({ content_id: String(cid2), content_type: type, url, entity_name: username, meta });
        };
      ...
      buildPost(n) {
        ...
        return {
          ...
          media_type: n.is_video || n.video_versions ? "video" : n.carousel_media || n.edge_sidecar_to_children ? "carousel" : "photo",
          caption: m.caption,
          ...
          music_author: m.music_author,
          likes_count: m.likes_count,
          comments_count: m.comments_count,
          taken_at: m.taken_at,
          video_duration: n.video_duration || null,
      ...
      async scrapeUserMedia(user, username, maxItems = IG_MAX_ITEMS) {
        ...
        if (tl && tl.edges) tl.edges.forEach((e) => {
          media.push(...this.extractMedia(e.node, username));
          posts.push(this.buildPost(e.node));
        });
        ...
        while (hasNext && media.length < maxItems) {
          ...
          const j = await fetchJson(url, { headers: this.headers(), credentials: "include" });
          (j.items || []).forEach((it) => {
            media.push(...this.extractMedia(it, username));
            posts.push(this.buildPost(it));
          });
          maxId = j.next_max_id;
      ...
      storyItemMedia(it, username, kind) {
        let url = null, type = "photo";
        if (it.video_versions && it.video_versions[0]) {
          url = it.video_versions[0].url;
          type = "video";
        } else if (it.image_versions2 && it.image_versions2.candidates && it.image_versions2.candidates[0]) {
      ...
      async getStoryTray() {
        ...
        const nameById = {};
        entries.forEach((t) => {
          nameById[String(t.user.pk)] = t.user.username || String(t.user.pk);
        });
        let saved = 0;
        for (let i = 0; i < ids.length; i += 20) {
          ...
          let j;
          try {
            j = await fetchJson("https://www.instagram.com/api/v1/feed/reels_media/?" + qs, { headers: this.headers(), credentials: "include" });
          } catch (e) {
            if (e instanceof WallError) throw e;
            continue;
          }
          for (const id of chunk) {
            ...
            if (media.length) {
              await send({ type: "ingest", platform: "instagram", username: nameById[id], items: media });
              saved += media.length;
            }
          }
          await hsleep(4e3);
      ...
      async getTagged(userId, username) {
        ...
        for (let page = 0; page < 3; page++) {
          ...
          (j.items || []).forEach((it) => {
            ...
            tags.forEach((t) => {
              if (t.user) users.push({ user_id: t.user.pk, username: t.user.username, display_name: t.user.full_name });
            });
          });
          maxId = j.next_max_id;
      ...
      async _expiring(user, username) {
        ...
        await hsleep(3500);
        try {
          const s = await this.getStories(user.id, username);
          if (s.length) {
            await send({ type: "ingest", platform: "instagram", username, items: s });
            saved += s.length;
            clog("info", `${username}: +${s.length} story media (expiring)`, "instagram");
          }
        } catch (e) {
          if (e instanceof WallError) throw e;
        }
        await hsleep(3500);
        try {
          const h = await this.getHighlights(user.id, username);
          if (h.length) {
            await send({ type: "ingest", platform: "instagram", username, items: h });
            saved += h.length;
            clog("info", `${username}: +${h.length} highlight media`, "instagram");
          }
        } catch (e) {
          if (e instanceof WallError) throw e;
        }
      ...
      async _deep(user, username, hop) {
        ...
        if (media.length) {
          await send({ type: "ingest", platform: "instagram", username, items: media });
          saved += media.length;
        }
        if (posts.length) {
          await send({ type: "posts", platform: "instagram", username, posts });
        }
        for (const p of posts.slice(0, 3)) {
          await hsleep(6e3);
          try {
            const cm = await this.getComments(p.platform_post_id);
            if (cm.length) await send({ type: "comments", platform: "instagram", post_id: p.platform_post_id, comments: cm });
          } catch (e) {
            if (e instanceof WallError) throw e;
          }
        }
        if (user.id) {
          ...
          try {
            ...
            if (tg.media.length) {
              await send({ type: "ingest", platform: "instagram", username, items: tg.media });
              saved += tg.media.length;
              clog("info", `${username}: +${tg.media.length} tagged media`, "instagram");
            }
        ...
        if (hop < 2 && user.id && Math.random() < 0.3) {
          await hsleep(5e3);
          ...
          if (found.length) {
            await send({ type: "discover", platform: "instagram", source: username, hop, discovered: found });
      ...
      async seedFromSelf() {
        ...
        try {
          ownerUsername = window._sharedData && window._sharedData.config && window._sharedData.config.viewer && window._sharedData.config.viewer.username || null;
        } catch (e) {
        }
      ...
      async runCycle() {
        ...
        if (left > 0) {
          clog("warn", `IG throttled \u2014 resting, ${Math.ceil(left / 6e4)}m left (not touching IG)`, "instagram");
          await sleep(Math.min(left, human(igRestWindowMs())));
        ...
        if (_curId && !IG_SEEDED_ACCOUNTS.has(_curId)) {
          IG_SEEDED_ACCOUNTS.add(_curId);
          this.seedFromSelf();
        }
        let resp = [];
        try {
          resp = await send({ type: "getTargets", platform: "instagram" }) || [];
        } catch (e) {
        }
        const pool = (Array.isArray(resp) ? resp : []).map((t) => typeof t === "string" ? { username: t, hop: 0 } : t).filter((t) => t && t.username);
        if (!pool.length) return { targets: 0, saved: 0, discovered: 0 };
        const cache = /* @__PURE__ */ new Map();
        const getUser = async (u) => {
          if (cache.has(u)) return cache.get(u);
          const p = await this.getProfile(u);
          cache.set(u, p);
          return p;
        };
        ...
        for (const t of deep) {
          ...
          try {
            ...
            if (!user) continue;
            if (!okProfile(user)) {
              clog("info", `skip famous ${t.username}`, "instagram");
              continue;
            }
            await this._sendProfile(user);
    ...
    function tiktokRevisitKey() {
      return "uc_tiktok_revisit_active";
    }
    function currentTikTokRevisit() {
      try {
        ...
        if (active.claimed_at && Date.now() - active.claimed_at > 30 * 6e4) {
    ...
    const BROWSER_MEDIA_REVISIT_PLATFORMS = /* @__PURE__ */ new Set(["instagram", "x", "threads", "facebook", "lemon8"]);
    function browserMediaRevisitKey(platform) {
      return "uc_browser_media_revisit_active_" + platform;
    }
    ...
    function browserMediaRevisitUrlOk(platform, url) {
      ...
      try {
        const u = new URL(url);
        if (platform === "x") {
          return /^(x|twitter)\.com$/i.test(u.hostname.replace(/^www\./, "")) && /^\/[A-Za-z0-9_]{1,20}\/status\/\d+/.test(u.pathname);
        }
        if (platform === "threads") {
          return /^(threads\.com|threads\.net)$/i.test(u.hostname.replace(/^www\./, "")) && /^\/@[^/]+\/post\//.test(u.pathname);
        }
        if (platform === "instagram") {
          return /(^|\.)instagram\.com$/i.test(u.hostname) && /^\/(?:p|reel|reels|tv|stories)\//.test(u.pathname);
        }
        if (platform === "facebook") {
          return /(^|\.)facebook\.com$/i.test(u.hostname) && (/\/(posts|photos|videos)\//.test(u.pathname) || /^\/(?:photo|permalink|story)\.php/i.test(u.pathname));
        }
        if (platform === "lemon8") {
          return /lemon8/i.test(u.hostname) && (/\/@[^/]+/.test(u.pathname) || /\/\d{6,}/.test(u.pathname));
        }
      } catch (e) {
      }
      return false;
    ...
    async function finishBrowserMediaRevisit(platform, active, response, observed, username) {
      ...
      const reason = observed > 0 ? attempted && accepted <= 0 ? "detail_page_upload_failed" : "detail_page_harvested" : "detail_page_no_media";
      await send({
        type: "browserMediaRevisitResult",
        platform,
        content_id: active.content_id,
    ...
    const tiktok = {
      ...
      async runCycle() {
        ...
        const owner = ownerFromStoredOrDom("tiktok", () => {
          const state2 = parseEmbeddedState(["__UNIVERSAL_DATA_FOR_REHYDRATION__", "SIGI_STATE", "sigi-persisted-data"]);
          const scope = state2 && state2.__DEFAULT_SCOPE__;
          return scope && scope["webapp.app-context"] && scope["webapp.app-context"].user && scope["webapp.app-context"].user.uniqueId || state2 && state2.AppContext && state2.AppContext.user && state2.AppContext.user.uniqueId || "";
        });
        ...
        const response = sink.items.length ? await send({ type: "ingest", platform: "tiktok", username: entity, items: sink.items }) : await send({
          type: "ingest",
          platform: "tiktok",
          username: entity,
          items: [],
    ...
    function lemon8HandleFromHref(href) {
      try {
        ...
        const reserved = /* @__PURE__ */ new Set(["feed", "foryou", "fashion", "beauty", "food", "travel", "home", "topic", "search"]);
    ...
    function lemon8TopicFromHref(href) {
      try {
        ...
        const topic = decodeURIComponent(parts[1]).trim().replace(/^#/, "").replace(/[^A-Za-z0-9_.-]/g, "_").slice(0, 80);
    ...
    function lemon8CollectDomUsers() {
      ...
      document.querySelectorAll('a[href*="/@"]').forEach((a) => {
        try {
          ...
          const avatar = [...a.querySelectorAll("img")].map((im) => ({
            im,
            ...
            try {
              ...
              const h = im.naturalHeight || im.height || 0;
              const r = im.getBoundingClientRect && im.getBoundingClientRect();
              return w && h && w <= 180 && h <= 180 || r && r.width && r.height && r.width <= 96 && r.height <= 96;
            } catch (e) {
              return false;
            }
          });
    ...
    function lemon8CollectDomMedia(sink, pageEntity) {
      const addCandidate = (url, el, role = "dom_image", contentType = "photo") => {
        ...
        let postUrl = null;
        try {
          postUrl = href ? new URL(href, location.href).href : null;
        } catch (e) {
        }
      ...
      document.querySelectorAll("video").forEach((v) => {
        try {
          if (v.poster) addCandidate(v.poster, v, "video_poster", "photo");
        } catch (e) {
        }
        try {
    ...
    const lemon8 = {
      ...
      async runCycle() {
        ...
        const ingestResponse = forcedRecovery ? (sendSideEffect(
          ingestPayload,
          "lemon8",
          `Lemon8 ${entity} forced media write`,
          { timeoutMs: 3e4 }
        ), null) : await send(ingestPayload, { timeoutMs: 45e3 });
        ...
        if (uniqueUsers.length) {
          ...
          if (forcedRecovery) {
            sendSideEffect(usersPayload, "lemon8", "Lemon8 author write", { timeoutMs: 12e3 });
          } else {
            await send(usersPayload, { timeoutMs: 25e3 });
          }
        }
    ...
    function xStatusHref(root) {
      try {
        ...
        return (link && (link.getAttribute("href") || "")).replace(/^https?:\/\/(?:www\.)?(?:x\.com|twitter\.com)/i, "").split("?")[0];
      } catch (e) {
        return "";
      }
    }
    ...
    function harvestXPosts(entity, feed) {
      ...
      xTweetRoots().forEach((art) => {
        try {
          ...
          const caption = textEl ? (textEl.innerText || "").trim().slice(0, 2e3) : null;
          const grp = art.querySelector('[role="group"][aria-label]');
          const al = grp ? grp.getAttribute("aria-label") || "" : "";
          const num = (re) => {
            const x2 = al.match(re);
            return x2 ? parseInt(x2[1].replace(/[,.\s]/g, ""), 10) : null;
          };
          ...
          const quote = (() => {
            const links = [...art.querySelectorAll('a[href*="/status/"]')].map((a) => (a.getAttribute("href") || "").replace(/^https?:\/\/(?:www\.)?(?:x\.com|twitter\.com)/i, "").split("?")[0]).map((h) => h.match(/^\/([A-Za-z0-9_]{1,20})\/status\/(\d+)/)).filter(Boolean).map((mm) => ({ author: mm[1], id: mm[2], href: `/${mm[1]}/status/${mm[2]}` }));
            return links.find((item) => item.id !== pid) || null;
          })();
          const reposted = (() => {
            const m2 = allText.match(/@([A-Za-z0-9_]{1,20})\s+reposted/i);
            return m2 ? m2[1] : null;
          })();
          posts.push({
            ...
            comments_count: num(/([\d,.]+)\s+repl/i),
            reposts_count: num(/([\d,.]+)\s+repost/i),
            likes_count: num(/([\d,.]+)\s+like/i),
            views_count: num(/([\d,.]+)\s+view/i),
            media_type: "tweet",
            taken_at: Number.isFinite(takenAtMs) ? Math.floor(takenAtMs / 1e3) : null,
    ...
    function allowForYouFeedPass(platform) {
      try {
        return lsGet("uc_allow_for_you_feed", "0") === "1" || lsGet("uc_" + platform + "_allow_for_you_feed", "0") === "1";
      } catch (e) {
        return false;
      }
    }
    async function xSelectTab(name) {
      try {
        const tab = [...document.querySelectorAll('[role="tab"], a[role="tab"]')].find((t) => (t.textContent || "").trim().toLowerCase() === name.toLowerCase());
        if (tab) {
    ...
    function xProfileCount(handle, suffix) {
      try {
        ...
        const a = [...document.querySelectorAll("a[href]")].find((el) => re.test((el.getAttribute("href") || "").replace(/^https?:\/\/(?:www\.)?x\.com/i, "")));
    ...
    function scrapeXProfile(handle) {
      ...
      const nameLines = (nameBlock && nameBlock.innerText || "").split("\n").map((s) => s.trim()).filter(Boolean);
      const displayName = nameLines.find((s) => !s.startsWith("@") && !/^(Follow|Following|Subscribe)$/i.test(s)) || null;
      const avatar = [...document.querySelectorAll('img[src*="profile_images"]')].map((im) => im.currentSrc || im.src).find(Boolean) || null;
      const external = (() => {
    ...
    function xStatusLinkCount() {
      try {
        ...
        document.querySelectorAll('a[href*="/status/"]').forEach((a) => {
          const href = (a.getAttribute("href") || "").replace(/^https?:\/\/(?:www\.)?(?:x\.com|twitter\.com)/i, "").split("?")[0];
    ...
    const x = {
      ...
      entity() {
        const m = location.pathname.match(/^\/([^/?#]+)/);
        return m && !/^(home|explore|notifications|messages|i|search)$/.test(m[1]) ? m[1] : "timeline";
      },
      async runCycle() {
        const entity = this.entity();
        const owner = ownerFromStoredOrDom("x", xLoggedInOwner);
        ...
        if (entity !== "timeline") {
          ...
          if (profile) {
            sendSideEffect(
              ...
              { timeoutMs: forcedRecovery ? 12e3 : 25e3 }
            );
        ...
        if (activeMediaRevisit && !forcedRecovery) {
          ingestResponse = await send(ingestPayload, { timeoutMs: 45e3 });
        } else if (sink.items.length === 0) {
          ingestResponse = await send(ingestPayload, { timeoutMs: forcedRecovery ? 12e3 : 25e3 }).catch((e) => {
        ...
        const hasProgress = xposts.length > 0 || sink.items.length > 0 || xu.length > 0 || profileSeen;
        if (hasProgress) {
          xClearZeroProgress();
    ...
    function harvestPermalinkPosts(linkRe, idFrom) {
      ...
      document.querySelectorAll("a[href]").forEach((a) => {
        ...
        let caption = "";
        try {
          caption = (box && box.innerText || "").trim().slice(0, 2200);
        } catch (e) {
        }
        if (!caption) return;
        byId.set(pid, {
    ...
    async function threadsSelectFeed(want) {
      try {
        ...
        const find = (txt) => [...document.querySelectorAll('div[role="button"],a[role="link"],span,div')].find((e) => (e.textContent || "").trim() === txt && e.offsetParent !== null);
        let item = find(want);
        if (!item) {
          const opener = find(other);
          if (opener) {
            opener.click();
    ...
    function thMarkNoAcct(user) {
      ...
      s.add(user);
      let arr = [...s];
      if (arr.length > 1e3) arr = arr.slice(-1e3);
    ...
    const threads = {
      ...
      async _scrapeProfile(user, forcedRecovery = false) {
        ...
        if (posts.length) {
          ...
          if (forcedRecovery) {
            sendSideEffect(postsPayload, "threads", `Threads @${user} forced post write`, { timeoutMs: 12e3 });
          } else {
      ...
      async runCycle() {
        ...
        if (/^\/@/.test(location.pathname)) {
          ...
          if (threadsProfileMissing()) {
            const activeMediaRevisit2 = currentBrowserMediaRevisit("threads");
            if (activeMediaRevisit2) {
              await finishBrowserMediaRevisit("threads", activeMediaRevisit2, null, 0, user);
            }
            thMarkNoAcct(user);
        ...
        const feed = allowForYouFeedPass("threads") && c % 4 === 0 ? await threadsSelectFeed("For you") ? "foryou" : "feed" : await threadsSelectFeed("Following") ? "following" : "feed";
        ...
        if (posts.length) {
          ...
          if (forcedRecovery) {
            sendSideEffect(postsPayload, "threads", `Threads ${feed} forced post write`, { timeoutMs: 12e3 });
          } else {
            await send(postsPayload);
          }
        ...
        if (authors.length) {
          ...
          if (forcedRecovery) {
            sendSideEffect(usersPayload, "threads", `Threads ${feed} forced user write`, { timeoutMs: 12e3 });
          } else {
        ...
        if (!forcedRecovery && c % 3 === 0) {
          try {
            ...
            const pool = (Array.isArray(resp) ? resp : resp.targets || []).map((t) => typeof t === "string" ? { username: t } : t).filter((t) => t && t.username);
    ...
    function scrapeFacebookProfile(entity, person) {
      ...
      const avatar = [...document.querySelectorAll("image, img")].map((im) => im.href && im.href.baseVal || im.currentSrc || im.src).find((u) => u && /fbcdn\.net/.test(u) && !/emoji|rsrc\.php|static/i.test(u)) || null;
      return {
    ...
    function facebookPostIdFromHref(href) {
      const s = String(href || "").replace(/^https?:\/\/(?:www\.)?facebook\.com/i, "").split("#")[0];
      const patterns = [
        /\/posts\/(pfbid[\w]+|\d{6,})/i,
    ...
    function facebookAuthorFromArticle(art, fallback) {
      try {
        const link = [...art.querySelectorAll("a[href]")].map((a) => {
    ...
    function harvestFacebookPosts(entity) {
      ...
      document.querySelectorAll('[role="article"], [data-pagelet*="FeedUnit"]').forEach((art) => {
        try {
          const text = String(art.innerText || "").replace(/\s+/g, " ").trim();
    ...
    const facebook = {
      ...
      _isPerson() {
        ...
        const txt = (document.body.innerText || "").slice(0, 2e4);
      ...
      async runCycle() {
        ...
        if (person) posts.forEach((p) => {
          if (!p.author_username) p.author_username = entity;
        });
        ...
        }, { timeoutMs: forcedRecovery ? 3e4 : 45e3 });
    ...
    const STRAVA_ROUTE_NAV_MIN_MS = 4 * 60 * 1e3;
    const STRAVA_ROUTE_NAV_MAX_WAIT_MS = 10 * 60 * 1e3;
    const STRAVA_ROUTE_VISIT_TTL_MS = 6 * 60 * 60 * 1e3;
    const STRAVA_STREAM_SEEN_TTL_MS = 10 * 60 * 1e3;
    const STRAVA_STREAM_ATTEMPT_TTL_MS = 12 * 60 * 1e3;
    ...
    const strava = {
      ...
      async runCycle() {
        ...
        if (waitMs > 0) {
          clog("info", `route queue paused ${(waitMs / 6e4).toFixed(1)}m before next activity`, "strava");
    ...
    const ONE_SHOT_STALE_MS = 8 * 60 * 1e3;
    ...
    function oneShotTimeoutMs(platformId) {
      return ONE_SHOT_TIMEOUT_MS_BY_PLATFORM[platformId] || 4 * 60 * 1e3;
    }
    function loopCycleTimeoutMs(platformId) {
      return LOOP_CYCLE_TIMEOUT_MS_BY_PLATFORM[platformId] || 6 * 60 * 1e3;
    }
    ...
    function scrapePassStaleMs(platformId) {
      return Math.max(loopCycleTimeoutMs(platformId), oneShotTimeoutMs(platformId)) + 60 * 1e3;
    }
          ...
          try {
            ...
            if (leftMs > 0) {
              ...
              if (!LAST_WALL_LOG_AT[p.id] || now - LAST_WALL_LOG_AT[p.id] > WALL_LOG_GAP_MS) {
                ...
                clog("warn", `${p.label} cooldown active for ${Math.ceil(leftMs / 6e4)}m; pausing this tab`, p.label);
              }
              await sleep(Math.min(leftMs, human(Math.min(passRestMs(p.id), 3e5))));
              continue;
            }
            ...
            if (shell) {
              ...
              await sleep(clicked ? human(12e3) : human(Math.max(3e4, Math.min(delay, 3e5))));
              continue;
            }
            markLoopProgress();
            if (SCRAPE_PASS_RUNNING) {
              ...
              if (ageMs > staleMs) {
                ...
                scheduleOneShotReload(p, `scrape pass stuck ${Math.ceil(ageMs / 6e4)}m`);
                await sleep(human(3e4));
              } else {
                await reportForcedCycleHealth(p, "scrape_pass_already_running", SCRAPE_PASS_REASON || "loop", {
                  scrape_pass_age_ms: ageMs,
                  stale_after_ms: staleMs
                });
                await sleep(human(45e3));
              }
              continue;
            }
            ...
            const stats = await withDeadline(
              cyclePromise,
              // one pass: IG = a few profiles; others = scrape current page
              timeoutMs,
              `${p.label} loop scrape pass timed out after ${Math.ceil(timeoutMs / 6e4)}m`,
              "UCLoopCycleTimeout"
            );
            ...
            await sleep(human(e && e.name === "UCLoopCycleTimeout" ? Math.min(passRestMs(p.id) * 2, 6e5) : 6e4));
          }
          markLoopProgress();
          await send({ type: "loopStatus", platform: p.id, label: p.label, running: true, url: location.href }).catch(() => {
          });
          await sleep(human(passRestMs(p.id)));
        }
      } finally {
        LOOP_RUNNING = false;
        LOOP_STARTED_AT = 0;
    ...
    async function runOneShotCycle(reason) {
      ...
      if (ONE_SHOT_RUNNING) {
        ...
        if (ageMs > staleMs) {
          clog("warn", `${p.label} forced pass appears stuck for ${Math.ceil(ageMs / 6e4)}m; reloading tab`, p.label);
      ...
      try {
        const leftMs = wallLeftMs(p.id);
        if (leftMs > 0) {
          clog("warn", `${p.label} forced pass skipped; cooldown active for ${Math.ceil(leftMs / 6e4)}m`, p.label);
        ...
        const stats = await withDeadline(
          cyclePromise,
          timeoutMs,
          `${p.label} forced scrape pass timed out after ${Math.ceil(timeoutMs / 6e4)}m`,
          "UCOneShotTimeout"
        );
    ...
    chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
      if (!msg) {
        try {
          sendResponse({ ok: false, reason: "empty_message" });
        } catch (_) {
        }
        return false;
      }
      if (!ucContentScriptCurrent()) {
        try {
          sendResponse({ ok: false, reason: "content_script_not_current" });
        } catch (_) {
        }
        return false;
      }
      if (msg.type === "ensureLoop" || msg.type === "scrapeCycle") {
        ...
        send({
          ...
          content_age_seconds: Math.round(progressAgeMs / 1e3),
          stale_after_ms: staleMs
        }).catch(() => {
        });
    ...
    window.addEventListener("message", (ev) => {
      ...
      const where = (() => {
        try {
          return (location.pathname.match(/^\/@?([^/?#]+)/) || [, "feed"])[1].slice(0, 30);
        } catch (e) {
          return "feed";
        }
      })();
      if (m.type === "posts" && Array.isArray(m.posts) && m.posts.length) {
        ...
        send({
          ...
          samples_shipped: m.samples_shipped
        }).catch(() => {
        });
