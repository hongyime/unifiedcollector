...
(() => {
  ...
  var lastDiag = null;
  var lastProbe = null;
  var lastAutoProbeAt = 0;
  var autoProbeInFlight = false;
  var expandedPlatformTabs = false;
  ...
  async function closeDuplicateControlTabs() {
    try {
      ...
      const controls = (tabs || []).filter((tab) => tab && tab.id != null && isControlTabsPage(tab.url)).sort((a, b) => {
  ...
  function ago(ts) {
    if (!ts) return "never";
    const s = Math.max(0, Math.floor((Date.now() - Number(ts)) / 1e3));
  ...
  function runtimeErrorText(e) {
    return chrome.runtime.lastError && chrome.runtime.lastError.message ? chrome.runtime.lastError.message : String(e && e.message ? e.message : e);
  }
  function messageTimeoutMs(msg) {
    const type = msg && msg.type;
    if (type === "refreshScraperTabs") return 18e4;
    if (type === "openAll" || type === "scrapeNow") return 6e4;
    if (type === "testIngest") return 45e3;
    if (type === "getPlatforms" || type === "diagnostics") return 15e3;
    return 1e4;
  }
  ...
  async function render() {
    ...
    list.innerHTML = platforms.map((p) => {
      const login = p.noLogin ? badge("login", "idle", "no login needed") : p.loggedIn === true ? badge("login", "ok", "logged in") : p.loggedIn === false ? badge("login", "bad", "not logged in") : badge("login", "idle", "login unknown");
      const tab = p.tabOpen ? badge("tab", "ok", "tab open") : badge("tab", "idle", "no tab");
      const scr = p.scraper ? `<span class="pill active">scraper active</span>` : `<span class="pill">login-ready</span>`;
      return `
        <div class="card">
          <div>
    ...
    $("note").innerHTML = `<b>Scrapers active:</b> ${scrapers}. Other platforms open for login now and will scrape automatically once their scraper is added. Auto-opened scraper tabs are unpinned by default; keep them logged in. Expanded tabs are ${expandedPlatformTabs ? "on" : "off"}.`;
  ...
  function renderDiagnostics(backgroundError) {
    ...
    if (backgroundError) {
      el.innerHTML = `<strong class="bad">Background worker unreachable.</strong><br>Options page version: ${escapeHtml(pageVersion)}. Chrome error: ${escapeHtml(backgroundError)}<br>Use Reload extension, then reopen this page. If this stays red, Chrome did not start the MV3 service worker.`;
      return;
    }
    const status = lastDiag && lastDiag.status || {};
    const logs = (lastDiag && lastDiag.log || []).slice(-8).map((e) => `${hhmmss(e.t)} [${e.level}] ${e.msg}`).join("\n");
    const workerFresh = status.swStartedAt && Date.now() - Number(status.swStartedAt) < 10 * 60 * 1e3;
    const probe = lastProbe ? lastProbe.ok ? `<span class="ok">${lastProbe.auto ? "auto" : "manual"} ingest ok</span> (${escapeHtml(lastProbe.base || "")})` : `<span class="bad">${lastProbe.auto ? "auto" : "manual"} ingest failed</span>: ${escapeHtml(lastProbe.error || "unknown")}` : "manual ingest not tested from this page";
    const bridgeHeartbeat = status.lastBridgeHeartbeatOkAt ? `bridge heartbeat ok ${escapeHtml(ago(status.lastBridgeHeartbeatOkAt))}` : "bridge heartbeat not confirmed";
    const bridgeProblem = status.lastBridgeHeartbeatError ? `<br><span class="bad">Bridge heartbeat error:</span> ${escapeHtml(status.lastBridgeHeartbeatError)}` : "";
    const scraperHeartbeat = status.lastScraperHeartbeatAt ? `scraper heartbeat ${escapeHtml(ago(status.lastScraperHeartbeatAt))}: ${Number(status.lastScraperHeartbeatSent || 0)}/${Number(status.lastScraperHeartbeatSeen || 0)} sent` : "scraper heartbeat not confirmed";
    el.innerHTML = `<strong>Extension ${escapeHtml(lastDiag && lastDiag.version || pageVersion)}</strong> <span class="${workerFresh ? "ok" : "warn"}">worker ${workerFresh ? "fresh" : "not fresh"}</span><br>Worker started ${escapeHtml(ago(status.swStartedAt))}; loop ping ${escapeHtml(ago(status.lastLoopPing))}; ingest ${escapeHtml(lastDiag && lastDiag.ingestBase || "")}<br>${bridgeHeartbeat}; ${scraperHeartbeat}${bridgeProblem}<br>Probe: ${probe}` + (logs ? `<pre>${escapeHtml(logs)}</pre>` : "");
  }
  async function maybeAutoProbe(backgroundError) {
    if (backgroundError || autoProbeInFlight) return;
    const now = Date.now();
    if (now - lastAutoProbeAt < 12e4) return;
    lastAutoProbeAt = now;
  ...
  async function handleUrlActions() {
    ...
    const openIds = (params.get("open") || "").split(",").map((s) => s.trim()).filter(Boolean);
