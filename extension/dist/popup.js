...
(() => {
  ...
  async function renderStatus() {
    ...
    $("tab").innerHTML = nTabs ? `<span class="dot ok"></span>${nTabs} open` : `<span class="dot bad"></span>none \u2014 open one below`;
    try {
    ...
    const fresh = ucStatus.lastLoopPing && Date.now() - ucStatus.lastLoopPing < 8 * 6e4;
    if (!nTabs) {
      ...
      const m = Math.max(1, Math.round((ucStatus.cooldownUntil - Date.now()) / 6e4));
      $("scrapeState").innerHTML = `<span class="dot warn"></span>cooling down ~${m}m (rate-limit)`;
    } else if (ucStatus.loopRunning && fresh) {
      $("scrapeState").innerHTML = `<span class="dot ok"></span>running${ucStatus.loopPlatform ? " \xB7 " + ucStatus.loopPlatform : ""}`;
    } else {
      $("scrapeState").innerHTML = `<span class="dot warn"></span>starting\u2026 keep the tab open`;
    }
    $("last").textContent = ucStatus.lastCycle ? `${ago(ucStatus.lastCycleAt)} \xB7 ${ucStatus.lastCycle.saved} media candidate(s), ${ucStatus.lastCycle.discovered} found` : "\u2014";
  ...
  async function load() {
    const { ingestBase: ingestBase2 } = await chrome.storage.local.get(["ingestBase"]);
    $("ingest").value = ingestBase2 || DEFAULT_INGEST;
    await refresh();
  }
  $("save").addEventListener("click", async () => {
    const ingestBase2 = $("ingest").value.trim() || DEFAULT_INGEST;
    await chrome.storage.local.set({ ingestBase: ingestBase2 });
    refresh();
  });
