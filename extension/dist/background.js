...
(() => {
  ...
  self.addEventListener("error", (event) => {
    ...
    try {
      console.error("[UC sw_error]", detail);
    } catch (_) {
    }
    try {
      _reportSwCrash(detail);
    } catch (_) {
    }
  });
  self.addEventListener("unhandledrejection", (event) => {
    ...
    try {
      console.error("[UC sw_reject]", detail);
    } catch (_) {
    }
    try {
      _reportSwCrash(detail);
  ...
  var BROWSER_UPLOAD_FETCH_TIMEOUT_MS = 45e3;
  var BROWSER_UPLOAD_POST_TIMEOUT_MS = 3e4;
  var BROWSER_UPLOAD_ATTEMPT_LIMIT_BY_PLATFORM = {
    instagram: 4,
    threads: 3,
    tiktok: 2,
    facebook: 2,
  ...
  var browserUploadChains = {};
  var PAGE_RECOVERY_PREFIX = "uc-page-recovery:";
  var PAGE_RECOVERY_STATE_KEY = "ucPageRecovery";
  var PAGE_RECOVERY_MAX_ATTEMPTS = 3;
  var PAGE_RECOVERY_STATE_TTL_MS = 60 * 60 * 1e3;
  var PAGE_RECOVERY_LIMIT_COOLDOWN_MS = 30 * 60 * 1e3;
  var PAGE_RECOVERY_LIMIT_COOLDOWN_MS_BY_PLATFORM = {
    x: 10 * 60 * 1e3
  };
  var RELOAD_INTENT_KEY = "ucReloadIntent";
  var LOADED_VERSION_KEY = "ucLoadedExtensionVersion";
  var SCRAPER_HEARTBEAT_TIMEOUT_MS = 12e3;
  var SCRAPER_HEARTBEAT_CONCURRENCY = 3;
  var PAGE_RECOVERY_DELAY_WINDOWS_MS = [
    [45e3, 15e4],
    [24e4, 48e4],
    [72e4, 12e5]
  ];
  ...
  async function sendManualIngestProbe() {
    ...
    try {
      const { response: r } = await postJsonWithTimeout(base + "/social/browser-heartbeat", payload, 6e3);
  ...
  async function reportScraperTabHeartbeats(reason) {
    ...
    await reportScraperHeartbeatSummary(base, reason, { seen, sent, failed, canonical, skipped, lastError }).catch(() => {
    });
  }
  async function reportScraperHeartbeatSummary(base, reason, summary) {
    ...
    const healthStatus = seen <= 0 ? "scraper_tabs_missing" : failed > 0 || sent <= 0 ? "scraper_heartbeat_degraded" : "scraper_heartbeat_ok";
  ...
  var lastForcedCycleByTab = {};
  var lastForcedReloadByTab = {};
  var lastForcedFailureByTab = {};
  var FORCED_CYCLE_DEBOUNCE_MS = 5 * 60 * 1e3;
  var FORCED_CYCLE_HARD_RELOAD_MS_BY_PLATFORM = {
    x: 5 * 60 * 1e3,
    facebook: 7 * 60 * 1e3,
    tiktok: 6 * 60 * 1e3,
    lemon8: 6 * 60 * 1e3,
  ...
  var FORCED_CYCLE_RELOAD_DEBOUNCE_MS = 4 * 60 * 1e3;
  var FORCED_CYCLE_FAILURE_DEBOUNCE_MS = 90 * 1e3;
  var TAB_MESSAGE_TIMEOUT_MS = 3e4;
  var MESSAGE_TIMEOUT_STALE_RELOAD_SECONDS_BY_PLATFORM = {
    facebook: 10 * 60,
    instagram: 10 * 60,
    lemon8: 10 * 60,
    x: 10 * 60,
  ...
  var EXTENSION_AUTO_RELOAD_STATE_KEY = "ucExtensionAutoReloadState";
  var EXTENSION_AUTO_RELOAD_DEBOUNCE_MS = 30 * 1e3;
  var EXTENSION_AUTO_RELOAD_DELAY_MS = 1200;
  var EXTENSION_AUTO_RELOAD_MAX_ATTEMPTS = 8;
  var EXTENSION_AUTO_RELOAD_ALARM = "uc-extension-auto-reload";
  function forcedCycleHardReloadMs(platformId) {
    return FORCED_CYCLE_HARD_RELOAD_MS_BY_PLATFORM[platformId] || 5 * 60 * 1e3;
  }
  function hardRefreshNavigationUrl(platform, currentUrl, now) {
    ...
    if (platform.id === "threads") {
      return `https://www.threads.com/?uc_recover=${Math.floor(now / 1e3)}`;
    }
  ...
  var POST_RELOAD_NUDGE_DELAY_MS_BY_PLATFORM = {
    facebook: 3e4,
    instagram: 25e3,
    lemon8: 25e3,
    threads: 45e3,
  ...
  var POST_RELOAD_NUDGE_RETRY_DELAY_MS_BY_PLATFORM = {
    threads: 6e4,
    tiktok: 9e4,
    x: 9e4
  };
  function postReloadScrapeNudgeDelayMs(platform, extra = {}) {
    ...
    return POST_RELOAD_NUDGE_DELAY_MS_BY_PLATFORM[platform && platform.id] || 12e3;
  }
  function postReloadScrapeNudgeRetryDelayMs(platform) {
    return POST_RELOAD_NUDGE_RETRY_DELAY_MS_BY_PLATFORM[platform && platform.id] || 45e3;
  }
    ...
    if (lastForcedAt && forcedAgeMs > hardReloadMs) {
      ...
      if (!lastReloadAt || now - lastReloadAt > FORCED_CYCLE_RELOAD_DEBOUNCE_MS) {
        try {
          ...
          await log("warn", `${platform.label}: stale forced scrape did not finish after ${Math.round(forcedAgeMs / 1e3)}s; hard-refreshed tab`);
        } catch (e) {
    ...
    try {
      ...
      if (messageTimedOut) {
        ...
        if (Number.isFinite(contentAgeSeconds) && contentAgeSeconds >= staleReloadSeconds) {
          ...
          if (!reloaded) {
            const refreshed2 = await refreshContentScriptAndNudge(base, tab, platform, "forced_cycle_reload_debounced_timeout", {
              content_age_seconds: contentAgeSeconds,
              stale_reload_seconds: staleReloadSeconds,
              cycle_error: cycleError,
              message_timeout: true,
      ...
      try {
        if (receiverMissing) {
          try {
            const reloaded2 = await refreshTabForMissingContentScript(base, tab, platform, "content_script_receiver_missing", {
              content_age_seconds: body.content_age_seconds || null,
              cycle_error: cycleError,
              no_receiver: true
            });
  ...
  var ALARM_REFRESH = "uc-refresh";
  var REFRESH_MIN = 45;
  var ALARM_MEMORY = "uc-memory-check";
  var MEMORY_SENSITIVE_PLATFORMS = /* @__PURE__ */ new Set(["facebook", "threads"]);
  var MEMORY_CHECK_INTERVAL_MIN = 30;
  var MEMORY_RELOAD_THRESHOLD_JS_MB = 250;
  var MEMORY_RELOAD_DOM_NODES = 4e4;
  var MEMORY_RELOAD_MIN_INTERVAL_MIN = 90;
  var MEMORY_TIME_CAP_MIN = 240;
  var MEMORY_LAST_RELOAD_KEY = "ucMemoryLastReloadByTab";
  var MEMORY_THRESHOLD_MB_OVERRIDE_KEY = "memoryReloadThresholdMB";
  var MEMORY_DOM_OVERRIDE_KEY = "memoryReloadDomNodes";
  async function autoTabsEnabled() {
    const { ucAutoTabs } = await chrome.storage.local.get("ucAutoTabs");
    return ucAutoTabs !== false;
  }
  var _sleep = (ms) => new Promise((r) => setTimeout(r, ms));
  var _jitterMs = (base, spread = 0.55) => Math.round(base * (1 - spread + Math.random() * spread * 2));
  var _humanGap = (base) => {
    let ms = _jitterMs(base, 0.65);
    if (Math.random() < 0.12) ms += 3e3 + Math.random() * 9e3;
    return Math.max(750, Math.round(ms));
  };
  var _randBetween = (min, max) => Math.round(min + Math.random() * (max - min));
  var _alarmJitter = () => _sleep(15e3 + Math.random() * 12e4);
  var _tabsOpInProgress = false;
  var _startupRecoveryChain = Promise.resolve();
  ...
  var _tpath = (u) => {
    try {
      return new URL(u).pathname.split("?")[0].replace(/\/$/, "") || "/";
    } catch (e) {
      return "/";
    }
  };
  var _mainWorldHookHostRe = /(^|\.)((instagram|tiktok|strava)\.com|threads\.com|x\.com|twitter\.com|facebook\.com)$/i;
  var SCRAPER_TABS_PINNED_KEY = "ucPinScraperTabs";
  var EXPANDED_PLATFORM_TABS_KEY = "ucOpenExpandedPlatformTabs";
  var AUTO_SCRAPER_PLATFORM_IDS = /* @__PURE__ */ new Set(["instagram", "threads", "tiktok", "x", "facebook", "strava"]);
  ...
  function scraperTabRows(tabs) {
    return (tabs || []).map((tab) => ({ tab, platform: platformForTabUrl(tab.url) })).filter((row) => row.platform && row.tab && row.tab.id != null);
  }
  ...
  async function closeDuplicateExtensionControlTabs(reason) {
    ...
    try {
      ...
      const controls = tabs.filter((tab) => tab && tab.id != null && isExtensionControlTabUrl(tab.url)).sort((a, b) => {
  ...
  function shouldNormalizeSingleFeedTab(p, tab, reason) {
    ...
    const NORMALIZE_PLATFORMS = /* @__PURE__ */ new Set(["instagram", "threads", "tiktok", "x", "facebook", "strava", "lemon8"]);
  ...
  async function scraperGroupHint() {
    try {
      ...
      for (const t of tabs) {
        const gid2 = Number(t.groupId);
        const wid2 = Number(t.windowId);
        if (!Number.isFinite(gid2) || gid2 < 0 || !Number.isFinite(wid2)) continue;
        const key = gid2 + "|" + wid2;
  ...
  async function createTabInSocialGroup(createOptions, hint) {
    const groupHint = hint === void 0 ? await scraperGroupHint() : hint;
    ...
    if (groupHint && Number.isFinite(groupHint.groupId) && created && created.id != null) {
      try {
        await chrome.tabs.group({ tabIds: [created.id], groupId: groupHint.groupId });
      } catch (e) {
      }
    }
    return created;
    ...
    try {
      ...
      for (const p of scraperPlatforms()) {
        ...
        if (targets.length <= 1) {
          if (tabs.length === 0) {
            try {
              await createTabInSocialGroup({ url: p.url, pinned: pinScraperTabs, active: false }, groupHint);
              opened++;
              await _sleep(_humanGap(3e3));
            } catch (e) {
            }
          } else {
            try {
              await setScraperTabPinnedState(tabs[0], pinScraperTabs);
            } catch (e) {
            }
            if (shouldNormalizeSingleFeedTab(p, tabs[0], reason)) {
              try {
                await chrome.tabs.update(tabs[0].id, { url: p.url });
                navigated++;
                await _sleep(_humanGap(3e3));
              } catch (e) {
              }
          ...
          for (const t of tabs) {
            ...
            if (m) {
              if (kept.has(m)) {
                ...
                kept.add(m);
                try {
                  await setScraperTabPinnedState(t, pinScraperTabs);
                } catch (e) {
                }
              }
            }
          ...
          for (let i = 0; i < targets.length; i++) {
            if (!kept.has(wantPaths[i])) {
              try {
                await createTabInSocialGroup({ url: targets[i], pinned: pinScraperTabs, active: false }, groupHint);
                opened++;
                await _sleep(_humanGap(3e3));
              } catch (e) {
              }
  ...
  async function syncCookies() {
    try {
      ...
      if (!cookies || !cookies.some((c) => c.name === "sessionid")) return;
  ...
  async function refreshScraperTabs(options = {}) {
    ...
    const gapMs = Number.isFinite(requestedGapMs) && requestedGapMs >= 250 ? requestedGapMs : 8e3;
    const pinScraperTabs = await scraperTabsPinned();
    let reloaded = 0;
    let errors = 0;
  ...
  async function fetchTabMemoryMetrics(tabId) {
    const reader = () => ({
      js_heap_bytes: typeof performance !== "undefined" && performance.memory && performance.memory.usedJSHeapSize || null,
      js_heap_limit_bytes: typeof performance !== "undefined" && performance.memory && performance.memory.jsHeapSizeLimit || null,
  ...
  async function checkMemorySensitiveTabs(reason = "scheduled") {
    ...
    const minIntervalMs = MEMORY_RELOAD_MIN_INTERVAL_MIN * 60 * 1e3;
    const timeCapMs = MEMORY_TIME_CAP_MIN * 60 * 1e3;
  ...
  function runStartupRecovery(reason, options = {}) {
    ...
    _startupRecoveryChain = _startupRecoveryChain.catch(() => null).then(() => performStartupRecovery(reason, { ...options, attempt })).catch((e) => log("warn", `${reason} recovery failed: ${e && e.message ? e.message : e}`));
  ...
  async function performStartupRecovery(reason, options = {}) {
    ...
    if (openTabs) {
      await ensureScraperTabsOpen(reason, { force }).catch((e) => log("warn", `${reason} tab audit failed: ${e && e.message ? e.message : e}`));
    }
    await reportBridgeHeartbeat(reason).catch((e) => log("warn", `${reason} bridge heartbeat failed: ${e && e.message ? e.message : e}`));
    await reportScraperTabHeartbeats(reason).catch((e) => log("warn", `${reason} scraper heartbeat failed: ${e && e.message ? e.message : e}`));
    if (refreshTabs) {
      const refreshGapMs = Number.isFinite(Number(options.refreshGapMs)) ? Number(options.refreshGapMs) : 1500;
      await refreshScraperTabs({ bypassCache: true, force: refreshForce || force, reason, gapMs: refreshGapMs }).catch((e) => log("warn", `${reason} tab refresh failed: ${e && e.message ? e.message : e}`));
      await reportScraperTabHeartbeats(reason + "_refresh").catch((e) => log("warn", `${reason} refresh heartbeat failed: ${e && e.message ? e.message : e}`));
    }
    await ensureLoops(reason).catch((e) => log("warn", `${reason} loop nudge failed: ${e && e.message ? e.message : e}`));
    if (attempt < retries) {
      const delay = [45e3, 15e4, 36e4][Math.min(attempt, 2)];
      setTimeout(() => {
  ...
  chrome.alarms.onAlarm.addListener(async (a) => {
    if (a.name && a.name.startsWith(PAGE_RECOVERY_PREFIX)) {
      await runPageRecovery(a.name.slice(PAGE_RECOVERY_PREFIX.length));
  ...
  function scraperUrlPatterns() {
    return scraperPlatforms().flatMap(platformUrlPatterns);
  }
  function platformById(id) {
    return scraperPlatforms().find((p) => p.id === id) || null;
  }
  function pageRecoveryAlarm(tabId) {
    return PAGE_RECOVERY_PREFIX + String(tabId);
  }
  ...
  async function clearPageRecoveryState(reason) {
    try {
      await chrome.storage.local.remove(PAGE_RECOVERY_STATE_KEY);
    } catch (e) {
    }
    try {
  ...
  async function clearPageRecoveryForTab(tabId, reason) {
    ...
    if (changed) {
      try {
        await chrome.alarms.clear(pageRecoveryAlarm(tabId));
      } catch (e) {
      }
  ...
  async function schedulePageRecovery(base, msg, sender) {
    ...
    if (limitUntil && now < limitUntil) {
      ...
      return {
        ...
        attempt: attempts,
        cooldown_mins: Math.ceil(delay2 / 6e4)
      };
    }
    if (limitUntil && now >= limitUntil) {
      attempts = 0;
    ...
    if (prev.nextAt && now < prev.nextAt && samePage) {
      const delay2 = Math.max(0, prev.nextAt - now);
      await recordPageHealth(base, msg, sender, {
      ...
      return { ok: true, scheduled: true, pending: true, attempt: attempts, delay_ms: delay2 };
    }
    if (attempts >= PAGE_RECOVERY_MAX_ATTEMPTS) {
      ...
      return { ok: true, scheduled: false, reason: "attempt_limit", attempt: attempts, cooldown_mins: Math.ceil(cooldownMs / 6e4) };
    }
    attempts += 1;
    ...
    await log("warn", `${platform.label} page looks stuck (${msg.reason || "error shell"}); reload scheduled in ${Math.round(delay / 1e3)}s (attempt ${attempts}/${PAGE_RECOVERY_MAX_ATTEMPTS})`);
    await recordPageHealth(base, msg, sender, {
      recovery_scheduled: true,
      recovery_attempt: attempts,
  ...
  async function ensureLoops(reason) {
    ...
    const tabs = selection.rows.map((row) => row.tab);
    if (!selection.seen) {
      await log("warn", `no scraper tab open \u2014 paused (${reason}). Open one via \u{1F5C2} Manage social tabs.`);
  ...
  function safeUploadFilename(item, blob) {
    ...
    const ext = mime.includes("png") ? ".png" : mime.includes("webp") ? ".webp" : mime.includes("gif") ? ".gif" : mime.includes("mp4") ? ".mp4" : mime.includes("jpeg") || mime.includes("jpg") ? ".jpg" : "";
  ...
  async function uploadBrowserMediaCandidates(base, msg, items) {
    ...
    if (attempted || deferredCandidates.length) {
      ...
      await log(
        ...
        `\u{1F4E6} ${msg.platform || "instagram"} \xB7 ${msg.username} \xB7 browser-upload ${accepted}/${attempted} accepted${detail}${deferredCandidates.length ? `, ${deferredCandidates.length} deferred` : ""}`
      );
  ...
  async function openOrFocus(p, { active = false } = {}) {
    try {
      ...
      if (tab) {
        ...
        return { id: p.id, focused: true, tabId: tab.id };
      }
  ...
  chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
    const fastReplyTypes = /* @__PURE__ */ new Set(["log", "tabReady", "loopStatus", "cycleReport"]);
    if (msg && fastReplyTypes.has(msg.type)) {
      ...
      (async () => {
        try {
          ...
          try {
            await log("error", `fast-path ${msg.type} failed: ${e && e.message ? e.message : e}`);
          } catch (_) {
          }
        }
      })();
        ...
        switch (msg.type) {
          ...
          case "ingest": {
            try {
              ...
              const uploadAndLog = async () => {
                ...
                await log("info", `\u{1F4E5} ${platform} \xB7 ${msg.username} \xB7 ${j.accepted ?? urlItems.length} URL media queued, ${upload.stored}/${upload.attempted} browser-uploaded`);
                return upload;
              };
          ...
          case "discover": {
            try {
              ...
              const j = await r.json().catch(() => ({}));
              await log("info", `\u{1F578} ${msg.platform || "instagram"} \xB7 spider from ${msg.source} (hop ${msg.hop}) \xB7 +${j.added ?? "?"} new accounts`);
          ...
          case "posts": {
            try {
              ...
              const j = await r.json().catch(() => ({}));
              await log("info", `\u{1F4DD} ${msg.platform || "instagram"} \xB7 ${msg.username} \xB7 ${j.saved ?? msg.posts.length} post(s) w/ captions+counts`);
              sendResponse({ ok: r.ok });
            } catch (e) {
              await log("error", `posts failed: ${e.message}`);
          ...
          case "comments": {
            try {
              ...
              const j = await r.json().catch(() => ({}));
              await log("info", `\u{1F4AC} ${msg.platform || "instagram"} \xB7 post ${msg.post_id} \xB7 ${j.saved ?? "?"} comment(s)`);
              sendResponse({ ok: r.ok });
            } catch (e) {
              await log("error", `comments failed: ${e.message}`);
              sendResponse({ ok: false });
            }
            break;
          }
          ...
          case "users": {
            try {
              ...
              const j = await r.json().catch(() => ({}));
              await log("info", `\u{1F464} ${msg.platform || "instagram"} \xB7 +${j.recorded ?? "?"} users (via ${msg.context || "seen"})`);
              sendResponse({ ok: r.ok });
            } catch (e) {
              await log("error", `users failed: ${e.message}`);
              sendResponse({ ok: false });
            }
            break;
          }
          case "dms": {
            try {
              ...
              const j = await r.json().catch(() => ({}));
              await log("info", `\u2709\uFE0F ${msg.platform || "instagram"} \xB7 +${j.recorded ?? "?"} DM messages`);
              sendResponse({ ok: r.ok });
            } catch (e) {
              await log("error", `dms failed: ${e.message}`);
              sendResponse({ ok: false });
            }
            break;
          }
          case "tiktok_dm":
          // DM JSON frame from a WS (rare) — capture (#35)
          case "instagram_dm": {
            try {
            ...
            try {
              await fetch(base + "/social/dm-probe", {
                ...
                body: JSON.stringify(withExtensionVersion({
                  ...
                  url: msg.url,
                  frame_kind: msg.frame_kind,
                  frame_size: msg.frame_size
                }))
              });
              await log("info", `\u{1F50E} ${msg.platform} DM ${msg.transport}: ${msg.frame_kind || "url"} ${msg.frame_size ? "~" + msg.frame_size + "B " : ""}${msg.url || ""}`);
            } catch (e) {
            }
            sendResponse({ ok: true });
            break;
          ...
          case "dm_sample": {
            try {
              ...
              await log("info", `\u{1F9EA} ${msg.platform} DM sample: ${msg.size}B`);
            } catch (e) {
            }
            sendResponse({ ok: true });
            break;
          ...
          case "dm_decoded": {
            try {
              ...
              if (n) await log("info", `\u{1F4E8} ${msg.platform} DM decoded: ${n} msg`);
            } catch (e) {
            }
            sendResponse({ ok: true });
            break;
          ...
          case "getStravaRouteQueue": {
            try {
              ...
              try {
                ...
                result = await fetchJsonWithTimeout(base + `/social/strava-route-queue${query}`, 3e4);
              }
          ...
          case "strava_streams": {
            try {
              ...
              const j = await r.json().catch(() => ({}));
              if (j.stored) await log("info", `\u{1F5FA} strava \xB7 activity ${msg.activity_id} \xB7 route ${j.point_count || pointCount} point(s) captured`);
              else if (j.rate_limit_recorded) await log("warn", `strava \xB7 activity ${msg.activity_id} \xB7 HTTP ${msg.http_status} route stream recorded`);
              else if (j.reason === "no_route_points") await log("warn", `strava \xB7 activity ${msg.activity_id} \xB7 no GPS points returned; pausing retries`);
              sendResponse({ ok: r.ok, ...j });
            } catch (e) {
          ...
          case "swFetchProxy": {
            try {
              ...
              if (path === "/social/browser-heartbeat" && sender && sender.tab && Number.isFinite(sender.tab.id)) {
              ...
              let parsedBody = null;
              try {
                parsedBody = proxied && proxied.body ? JSON.parse(proxied.body) : null;
              } catch (_) {
              }
              sendResponse({
          ...
          case "wall": {
            ...
            await setStatus({ cooldownUntil: Date.now() + mins * 6e4 });
            const account = String(msg.account || msg.owner || "").trim();
          ...
          case "testIngest": {
            sendManualIngestProbe().then((result) => {
              reportScraperTabHeartbeats("manual_ingest_probe").catch((e) => log("warn", `manual scraper heartbeat fan-out failed: ${e && e.message ? e.message : e}`));
              return result;
            }).then(sendResponse);
            return;
          }
          ...
          case "openAll": {
            ...
            for (const p of scraperPlatforms()) {
              const r = await openOrFocus(p, { active: first });
              first = false;
              if (r.opened) opened++;
              else if (r.focused) focused++;
              else errors++;
            }
          ...
          case "refreshScraperTabs": {
            ...
            await reportScraperTabHeartbeats("manual_refresh_tabs").catch((e) => log("warn", `manual refresh heartbeat failed: ${e && e.message ? e.message : e}`));
            sendResponse(result);
            break;
          }
          case "scrapeNow":
  ...
  async function scrapeNow() {
    ...
    setTimeout(() => {
      ensureLoops("manual").catch((e) => log("warn", `manual loop nudge failed: ${e && e.message ? e.message : e}`));
    }, 0);
  ...
  async function consumeReloadIntent() {
    ...
    try {
      await chrome.storage.local.remove(RELOAD_INTENT_KEY);
    } catch (e) {
    }
    if (!Number.isFinite(ageMs) || ageMs < 0 || ageMs > 10 * 60 * 1e3) {
    ...
    setTimeout(() => {
      (async () => {
        ...
        if (intent && intent.force_scrape) {
          await ensureLoops("manual_extension_reload_scrape").catch((e) => log("warn", `reload intent scrape nudge failed: ${e && e.message ? e.message : e}`));
        }
        if (intent && intent.force_test) {
          await sendManualIngestProbe().catch((e) => log("warn", `reload intent ingest probe failed: ${e && e.message ? e.message : e}`));
          await reportScraperTabHeartbeats("manual_extension_reload_probe").catch((e) => log("warn", `reload intent scraper heartbeat failed: ${e && e.message ? e.message : e}`));
        }
  ...
  (async () => {
    try {
      ...
      const consumed = await consumeReloadIntent().catch((e) => {
      ...
      if (!consumed) {
        if (versionState.changed) {
          ...
          setTimeout(() => {
            runStartupRecovery("version_changed", { force: true, refreshTabs: true, refreshGapMs: 1500, retries: 3 }).catch((e) => log("warn", `version-change recovery failed: ${e && e.message ? e.message : e}`));
          }, 500);
        } else {
      ...
      try {
        _reportSwCrash({ kind: "warm_start_throw", message: e && e.message, stack: e && e.stack });
      } catch (_) {
      }
      try {
        await log("warn", `warm-start crashed: ${e && e.message ? e.message : e}`);
      } catch (_) {
      }
    }
  })();
