...
export default function SystemHealth() {
  ...
  return (
    <div className="flex flex-col min-h-full">
      <Header
        title="System Health"
        subtitle="Live service status via WebSocket"
        lastUpdated={lastUpdated}
      />
      <div className="p-6 grid grid-cols-12 gap-3">

        {/* WS status bar — full width */}
        <div className="col-span-12 card p-3 flex items-center gap-4">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm text-text-secondary">
              {connected ? 'Live — updates every 5s' : 'Disconnected — reconnecting…'}
            </span>
          </div>
          {services.length > 0 && (
        ...
        {services.length === 0 ? (
          <div className="col-span-12 card p-16 flex items-center justify-center">
            <div className="text-center space-y-2">
              ...
              <p className="text-text-muted text-sm">
                {connected ? 'Waiting for health data…' : 'Connecting to WebSocket…'}
              </p>
            </div>
          </div>
        ) : (
          <>
            ...
            {/* Core services grid — left 8 cols */}
            <div className="col-span-12 lg:col-span-8 card p-4">
              <h2 className="text-text-muted text-xs uppercase tracking-wider mb-3">Core Services</h2>
            ...
            {/* Latency bars — right 4 cols */}
            <div className="col-span-12 lg:col-span-4 card p-4">
              <h2 className="text-text-muted text-xs uppercase tracking-wider mb-3">Latency Overview</h2>
                      ...
                      <div className="flex-1 h-1.5 bg-bg-elevated rounded-full overflow-hidden">
                        <div
                          className={[
                            ...
                            svc.status === 'down'
                              ? 'bg-status-down'
                              : svc.latency_ms && svc.latency_ms > 500
                              ? 'bg-status-pending'
                              : 'bg-white/40',
                          ].join(' ')}
                          style={{ width: `${pct}%` }}
            ...
            {/* WA clients — full width, only when present */}
            {services.some((s) => s.service.startsWith('wa-client')) && (
              <div className="col-span-12 card p-4">
                <h2 className="text-text-muted text-xs uppercase tracking-wider mb-3">WA Clients</h2>
