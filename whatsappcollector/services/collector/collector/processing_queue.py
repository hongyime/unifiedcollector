...
class RabbitMQBroker:
    ...
    async def declare_topology(self) -> None:
        ...
        queue_bindings = {
            "messages.inbound": ["msg.text", "msg.media.*"],
            "messages.status": ["msg.status"],
            "messages.history": ["messages.history", "history.sync"],
            "contacts.update": ["contacts.update"],
            "groups.metadata": ["groups.update", "groups.participants.update"],
...
class RedisStreamBroker:
    ...
    async def declare_topology(self) -> None:
        ...
        for stream in [
            ...
            try:
                await self.redis.xgroup_create(stream, "collector", id="$", mkstream=True)
                ...
                for message_id, fields in items:
                    ...
                    class _Message:
                        ...
                        async def ack(self_nonlocal):
                            await self.redis.xack(queue_name, "collector", message_id)
