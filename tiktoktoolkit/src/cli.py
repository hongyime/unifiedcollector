...
@download.command('watchlist')
@click.option('--file', 'usernames_file', default=None, help='Watchlist file (default: data/usernames.txt)')
@click.option('--limit', default=30, show_default=True, help='Max videos per user')
@click.option('--out', default=None, help='Output directory (will prompt if not provided)')
@click.option('--type', 'download_type', type=click.Choice(['videos', 'profile_pictures']), default='videos')
@click.pass_context
def download_watchlist_cmd(ctx, usernames_file, limit, out, download_type):
    """Download media for your curated watchlist (data/usernames.txt).

    These are the accounts you personally follow/track.
    For spider-discovered accounts use: download queue
    """
    downloader = ctx.obj['downloader']
    base_path = ctx.obj['base_path']

    watch_file = Path(usernames_file) if usernames_file else base_path / 'data' / 'usernames.txt'
    if not watch_file.exists():
        raise click.ClickException(
            f"Watchlist file not found: {watch_file}\n"
            "  Create it with one username per line, or pass --file <path>"
        )
    ...
    if not out:
        out = prompt_for_download_path(
            context=f"TikTok {download_type.replace('_', ' ')} (watchlist)",
            out_path=None,
            default_path=str(base_path / 'downloads'),
        )
    output_path = Path(out)

    click.echo(f"Reading watchlist from {watch_file}...")
    try:
        results = downloader.download_users_from_file(str(watch_file), limit, output_path, download_type=download_type)
    except FileNotFoundError as exc:
        raise click.ClickException(f"File not found: {exc}") from exc
    except Exception as exc:
        raise click.ClickException(f"Watchlist download failed: {exc}") from exc

    total_dl = sum(_count_results(r)['downloaded'] for r in results.values())
    total_skip = sum(_count_results(r)['skipped'] for r in results.values())
    total_fail = sum(_count_results(r)['failed'] for r in results.values())
    click.echo(f"\nWatchlist complete: {len(results)} users — {total_dl} downloaded, {total_skip} skipped, {total_fail} failed")
...
@cli.command('spider')
@click.option('--seed', is_flag=True, help='Enqueue seed accounts and spider their following lists')
@click.option('--batch', is_flag=True, help='Spider all pending users in queue')
@click.option('--username', help='Spider a single user (or comma-separated list)')
@click.option('--file', 'usernames_file', default=None, help='Text file of usernames to seed (one per line); defaults to data/usernames.txt when --seed is used')
@click.option('--limit', default=500, help='Maximum users to spider in batch mode')
@click.pass_context
def spider_command(ctx, seed, batch, username, usernames_file, limit):
    """Spider TikTok users to discover followers/following.

    Seed accounts go straight into the DB — no text file required.
    Pass --username for one or a few accounts, or --file to import a list.
    """
    ...
    try:
        if username:
            names = [u.strip().lstrip('@') for u in re.split(r'[,\s]+', username) if u.strip()]
            if len(names) == 1:
                click.echo(f"Spidering user: @{names[0]}")
                spider.enqueue(names)
                processed = spider.run_batch(max_items=1)
                click.echo(f"Successfully spidered @{names[0]}" if processed else f"Failed to spider @{names[0]}")
            else:
                click.echo(f"Spidering {len(names)} users...")
                spider.enqueue(names)
            ...
            # Resolve seed source: --file > data/usernames.txt > error
            if usernames_file:
                seed_file = Path(usernames_file)
            else:
                seed_file = base_path / 'data' / 'usernames.txt'

            if not seed_file.exists():
                raise click.ClickException(
                    f"Seed file not found: {seed_file}\n"
                    "  Pass usernames directly:  python main.py spider --username user1,user2\n"
                    "  Or specify a file:        python main.py spider --seed --file mylist.txt"
                )

            from .utils import read_usernames_from_file
            seed_users = read_usernames_from_file(str(seed_file))
            if not seed_users:
                raise click.ClickException(f"No valid usernames found in {seed_file}")

            click.echo(f"Seeding {len(seed_users)} accounts from {seed_file}")
            ...
            click.echo(f"Enqueued {added} new accounts ({len(seed_users) - added} already in DB)")
            click.echo("Fetching profiles and following lists for seed accounts...")

            total = spider.run_until_done()
            click.echo(f"\nSeed complete: {total} accounts processed")
            click.echo("Run 'python main.py download queue' to download all discovered accounts")

        else:
            click.echo("Must specify --seed, --batch, or --username")
            click.echo("  python main.py spider --username user1,user2   # Seed from CLI")
            click.echo("  python main.py spider --seed                   # Seed from data/usernames.txt")
            click.echo("  python main.py spider --seed --file list.txt   # Seed from custom file")
...
@click.option('--out', default=None, help='Output directory (will prompt if not provided)')
@click.option('--type', 'download_type', type=click.Choice(['videos', 'profile_pictures']), default='videos')
@click.option('--max-users', default=0, help='Stop after this many users (0 = unlimited)')
@click.option('--retry-failed', is_flag=True, help='Also retry users previously marked as failed')
@click.pass_context
def download_queue_cmd(ctx, limit, out, download_type, max_users, retry_failed):
    """Download media for all accounts discovered by the spider.

    Reads directly from the profiles DB (download_status='pending'),
    downloads each user, then marks them done or failed in the DB.
    ...
    if not out:
        base_path = ctx.obj['base_path']
        out = prompt_for_download_path(
            context=f"TikTok {download_type.replace('_', ' ')} (queue download)",
            out_path=None,
            default_path=str(base_path / 'downloads'),
        )
    output_path = Path(out)
    ...
    if not db_path.exists():
        raise click.ClickException(f"Profiles DB not found: {db_path}. Run 'spider --seed' first.")
    ...
    with _sqlite3.connect(str(db_path), timeout=5.0) as conn:
        rows = conn.execute(
            f"SELECT username FROM profiles WHERE download_status IN ({placeholders}) ORDER BY last_scraped_ts ASC",
            statuses,
        ).fetchall()

    usernames = [r[0] for r in rows]
    if not usernames:
        click.echo("No accounts pending download. Run 'python main.py spider --batch' to discover more.")
        return

    if max_users and max_users > 0:
        usernames = usernames[:max_users]

    click.echo(f"Downloading {download_type} from {len(usernames)} queued accounts (limit {limit} per user)...")

    done = failed = 0
    for i, uname in enumerate(usernames, 1):
        if resilience.is_shutdown():
            click.echo("\nShutdown requested — stopping queue.")
            break

        click.echo(f"[{i}/{len(usernames)}] @{uname}")
        try:
            results = downloader.download_user(uname, limit, output_path, download_type=download_type)
            counts = _count_results(results)
            new_status = 'done'
            done += 1
            click.echo(f"  downloaded={counts['downloaded']} skipped={counts['skipped']} failed={counts['failed']}")
        except Exception as exc:
            logger.warning(f"Queue download failed for @{uname}: {exc}")
            new_status = 'failed'
            failed += 1
        ...
        with _sqlite3.connect(str(db_path), timeout=5.0) as conn:
            conn.execute(
                "UPDATE profiles SET download_status=? WHERE username=?",
                (new_status, uname),
            )
            conn.commit()

    click.echo(f"\nQueue complete: {done} done, {failed} failed, {len(usernames) - done - failed} skipped (shutdown)")
