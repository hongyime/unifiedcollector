...
echo   ---  ------------------------------  ---------------  ------
echo   17   Quick Actions menu
echo   18   Run custom command
echo    0   Exit
echo.
...
if "%choice%"=="17" goto QUICK_ACTIONS
if "%choice%"=="18" goto CUSTOM_CMD
...
:ANALYZE
cls
echo  Analyze Collected Data [offline]
echo  ================================
echo.
...
echo  ===========================
echo.
"%PYTHON_EXE%" -c "import sys; sys.path.insert(0,'lib'); from config import INSTAGRAM_ACCOUNTS; names=','.join(a['name'] for a in INSTAGRAM_ACCOUNTS); print(f'  Your accounts: {names}')" 2>nul
set "account="
...
echo   6  Clear spider progress
echo   7  Clear download progress
echo   8  Clear ALL progress
echo   0  Back
echo.
set "pchoice="
...
"%PYTHON_EXE%" -c "import sys; sys.path.insert(0,'lib'); from config import INSTAGRAM_ACCOUNTS; [print(f'    {i+1}. {a[\"name\"]}') for i,a in enumerate(INSTAGRAM_ACCOUNTS)]"
echo.
