...
5. **Generic cross-platform link-extractor + reconciler modules** — `src/core/link_extractor.py` and `change_tracker.py` are WhatsApp-only despite generic names. Generalizing unlocks #6, #7 and the telegram/lemon8 reconciler gaps. Add `src/core/reconciler.py` (tier1 file-existence + tier2 sha256 rehash, parameterized by table/dir).
6. **YouTube description-link scraping** (`re.findall(r'https?://...', description)`) → feed link extractor. Old `video_processor.py:463`. Target `youtube/__init__.py` `_upsert_video`.
7. **Telegram multi-platform link extraction** (`extract_multi_platform_links`: whatsapp/discord/fb/ig/twitter/reddit). Target `telegram/__init__.py` `_upsert_message`.
