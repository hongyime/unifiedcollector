"""Concrete :class:`TelegramCollector` — composes the mixins with
:class:`~src.core.base_collector.BaseCollector`.

Extracted from :mod:`src.collectors.telegram` (PERF-004 sub-plan 4C step 10).
``src/collectors/telegram/__init__.py`` re-exports :class:`TelegramCollector`
from here so the historic import surface (``from src.collectors.telegram
import TelegramCollector``) keeps working unchanged.
"""
...
def _vault_root():
    """Resolve ``VAULT_ROOT`` via the parent :mod:`src.collectors.telegram`
    module so tests can monkeypatch ``tg_mod.VAULT_ROOT`` to redirect writes.
    Falls back to the module-local binding when the parent is unavailable
    (e.g. during interpreter shutdown)."""
    try:
        from src.collectors import telegram as _tg_shell
        return getattr(_tg_shell, "VAULT_ROOT", VAULT_ROOT)
    except Exception:
        return VAULT_ROOT


def _write_raw_payload(**kwargs):
    """Route ``write_raw_payload`` through the parent module so tests can
    monkeypatch ``tg_mod.write_raw_payload`` to intercept archive writes."""
    try:
        from src.collectors import telegram as _tg_shell
        fn = getattr(_tg_shell, "write_raw_payload", write_raw_payload)
    except Exception:
        fn = write_raw_payload
    return fn(**kwargs)

# ──────────────────────────────────────────────────────────────────────────
# Module-level helpers live in ``helpers.py`` (PERF-004 sub-plan 4C step 2).
# Re-exported here so the pre-split import surface — used by
# ``tests/collectors/test_telegram*.py`` — is preserved verbatim.
# ──────────────────────────────────────────────────────────────────────────
...
# ──────────────────────────────────────────────────────────────────────────
# SessionState + TelegramWorker + exceptions live in ``session.py``
# (PERF-004 sub-plan 4C step 3). Re-exported here so the pre-split import
# surface — used by ``tests/collectors/test_telegram*.py`` — is preserved.
# ──────────────────────────────────────────────────────────────────────────
from src.collectors.telegram.session import (  # noqa: E402,F401 — re-export
...
# ──────────────────────────────────────────────────────────────────────────
# Mixins own one concern each of the collector — extracted in PERF-004
# sub-plan 4C steps 4–9. TelegramCollector composes them via multiple
...
from src.collectors.telegram.mixins.media import MediaMixin
from src.collectors.telegram.mixins.members import MembersMixin
from src.collectors.telegram.mixins.profile import ProfileMixin
