...
@pytest.mark.asyncio
async def test_browser_media_zero_store_window_degrades_source(monkeypatch):
    from src.core import source_freshness

    class BrowserMediaZeroStoreConn:
        async def fetch(self, query: str, *args, timeout: int = 8):
            if "FROM source_health" in query:
                return []
            if "endpoint = 'browser_heartbeat'" in query:
    ...
    rows = await source_freshness.compute_liveness(BrowserMediaZeroStoreConn())
...
@pytest.mark.asyncio
async def test_x_manual_mode_does_not_degrade_system_health(monkeypatch):
    from src.core import source_freshness
                ...
                return [
                    {
                        "source": "x",
                        "status": "degraded",
                        "last_error": "browser capture stalled: old watchdog marker",
                        "last_success_at": datetime.now(timezone.utc) - timedelta(days=5),
                        "updated_at": datetime.now(timezone.utc),
                    }
                ]
            if "endpoint = 'browser_heartbeat'" in query:
    ...
    monkeypatch.setenv("X_SOURCE_MANUAL_MODE", "1")
    monkeypatch.setattr(
        source_freshness,
        "FRESHNESS",
        [("x", "SELECT extract(epoch FROM now()-max(collected_at)) FROM x_posts", 3600)],
    )

    rows = await source_freshness.compute_liveness(XManualConn())

    assert rows[0]["status"] == "live"
    assert rows[0]["browser_health_status"] == "manual_browser_source"
    assert "manual browser mode" in rows[0]["detail"]
