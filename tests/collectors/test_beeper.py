...
@pytest.mark.asyncio
async def test_collect_marks_quiet_message_sync_healthy_when_other_phases_empty(monkeypatch, tmp_path):
    monkeypatch.setenv("BEEPER_DESKTOP_API_TOKEN", "x")
    monkeypatch.setenv("COLLECTOR_DRIVE_PATH", str(tmp_path))
    ...
    coll = BeeperCollector(client=MagicMock(spec=BeeperClient))
    coll.set_pool(pool)
    coll._sync_accounts = AsyncMock(return_value=0)
    coll._sync_chats = AsyncMock(return_value=0)
    coll._sync_messages = AsyncMock(return_value=0)
