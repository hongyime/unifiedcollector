...
def test_browser_heartbeat_handler_requests_extension_reload_for_old_version(monkeypatch):
    monkeypatch.setattr(ig_ingest, "UC_EXTENSION_EXPECTED_VERSION", "1.23.4")
    req = _FakeRequest(
        {"pool": None},
        {
            "platform": "bridge",
    ...
    assert resp.status == 200
    payload = json.loads(resp.text)
    assert payload["expected_extension_version"] == "1.23.4"
    assert payload["current_extension_version"] == "1.23.3"
