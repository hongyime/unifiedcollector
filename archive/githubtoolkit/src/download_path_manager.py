...
def _handle_sigint(signum, frame):
    """Cleanup state and propagate KeyboardInterrupt for graceful shutdown."""
    _cleanup_session()
    signal.default_int_handler(signum, frame)


def _handle_sigterm(signum, frame):
    """Cleanup state and terminate with standard SIGTERM exit code."""
    _cleanup_session()
    raise SystemExit(143)


# atexit cleanup is always safe (runs after event loop closes)
atexit.register(_cleanup_session)

# Signal handlers are only registered when run as a standalone script.
# When imported as a library inside asyncio, registering SIGINT here
# overrides asyncio's own handler and causes RuntimeError on Windows.
