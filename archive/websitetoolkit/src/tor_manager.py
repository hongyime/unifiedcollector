"""
tor_manager.py — Tor daemon management for Website Toolkit.
...
Adapted from FORGE's forge/opsec/tor.py for use in Website Toolkit.
"""
...
class TorManager:
    """Manages the lifecycle of the portable Tor daemon for Website Toolkit."""
    ...
    def _find_tor_exe() -> Path:
        ...
        # Search in the websitetoolkit directory (where this script lives)
        ...
        raise FileNotFoundError(
            ...
            "Please ensure it is present or a valid archive is available in the websitetoolkit directory."
        )
    ...
    def _extract_tor_archive() -> bool:
        ...
        _LOG.error("No usable Tor archive found in websitetoolkit directory.")
        return False
    ...
    def rotate_circuit(self) -> bool:
        ...
        try:
            # Send HUP signal to Tor to rotate circuit
            import signal
            self._process.send_signal(signal.CTRL_BREAK_EVENT if os.name == "nt" else signal.SIGHUP)
