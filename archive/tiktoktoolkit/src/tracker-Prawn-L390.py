...
class SQLiteDownloadTracker(TrackerBackend):
    def __init__(self, db_path: Path, json_backup: Optional[JSONBackup] = None,
        ...
        # Migration removed - DB already migrated
        self._ensure_schema()
    ...
    def mark_downloaded(self, username: str, video_id: str, filepath: Optional[str] = None,
        ...
        with self._get_conn() as conn:
            cur = conn.execute("SELECT first_downloaded FROM videos WHERE username=? AND video_id=?", (username, video_id))
            row = cur.fetchone()
            if row:
                if file_hash:
                    conn.execute("""
                        UPDATE videos SET last_seen=?, filepath=COALESCE(?, filepath), size=COALESCE(?, size), hash=COALESCE(?, hash)
                        WHERE username=? AND video_id=?
                    """, (now, filepath, size, file_hash, username, video_id))
                else:
                    conn.execute("""
                        UPDATE videos SET last_seen=?, filepath=COALESCE(?, filepath), size=COALESCE(?, size)
                        WHERE username=? AND video_id=?
                    """, (now, filepath, size, username, video_id))
            else:
                conn.execute("""
