...
class BackfillRunner:
    ...
    def recommended_command(self, steps: int = 10) -> list[str]:
        command = [sys.executable, "-m", "ingestion.main", "--backfill-only", "--backfill-steps", str(steps)]
    ...
    def start(self, steps: int = 10) -> dict:
        if self.is_running():
            return self.status()
        ...
        self.log_path = settings.log_dir / f"backfill_continue_{timestamp}.log"
        self.command = self.recommended_command(steps)
