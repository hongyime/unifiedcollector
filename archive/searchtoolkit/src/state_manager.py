...
class StateManager:
    ...
    def _get_connection(self) -> sqlite3.Connection:
        """Get a database connection with WAL mode and busy retry enabled."""
    ...
    def _atexit_handler(self) -> None:
        """Checkpoint WAL on clean exit so DB file is up-to-date."""
        if self._shutdown:
            return
        self._shutdown = True
        try:
    ...
    def close(self) -> None:
        """Explicit cleanup — checkpoint WAL and mark done."""
    ...
    def mark_download_pending(self, url: str, query: str) -> bool:
        """Atomically claim a URL for download.

        Returns True if this caller owns the download (should proceed).
        Returns False if the URL is already completed, pending (another thread/process
        has it), or skipped — caller should skip without downloading.

        Failed URLs are re-claimed (returns True) so they get retried.
        """
        conn = self._get_connection()
        try:
            cursor = conn.execute(
                """INSERT INTO downloads (url, content_hash, query, status, created_at)
                   VALUES (?, '', ?, 'pending', ?)
            ...
            conn.commit()
            return cursor.rowcount > 0  # 1 = inserted or updated (we own it), 0 = skip
        finally:
            conn.close()
