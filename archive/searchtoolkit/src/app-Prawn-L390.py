...
def download_file(url, target_folder, base_filename, expected_type, query=""):
    ...
    # Check if already downloaded (state persistence)
    if _state_manager and _state_manager.is_downloaded(url):
        print(f"{Fore.YELLOW}  ⏭️  Already downloaded: {url[:60]}...{Style.RESET_ALL}")
        return True
    
    # Mark as pending
    if _state_manager:
        _state_manager.mark_download_pending(url, query)
