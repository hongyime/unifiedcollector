...
- Still not complete: Instagram headless/httpx raw archive coverage and restore replay from a real DB dump. GitHub bulk avatar range writes artifact sidecars only and intentionally does not create `media_items` rows for unknown numeric IDs.
