...
- Beeper, Telegram, WhatsApp, Lemon8, YouTube, TikTok, and Website generic media downloads now write physical bytes through `write_atomic_artifact` to canonical `media/blobs/<sha256>` paths while retaining per-occurrence `media_items` rows and sidecars. The collector dashboard media resolver accepts both legacy media-root paths and vault-backed blob paths.
...
- Still not complete: migrating remaining Instagram, Website streaming video, Search, GitHub avatar/media, Strava photo/map, shared profile-photo, and generic legacy helper paths through `write_atomic_artifact`; source-by-source migration proof; full Tier 1 raw payload coverage; and restore replay from a real DB dump.
